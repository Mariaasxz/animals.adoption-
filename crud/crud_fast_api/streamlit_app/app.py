...
st.set_page_config(page_title="CRUD pets ", layout="centered")
st.title(" Gestão de animais - ")

# --- Painel de Operações ---
st.subheader("Adicionar / Modificar Item")
with st.form("form_item", clear_on_submit=True):
    id_item = st.number_input("idade do animal  (Necessário apenas para Atualizar/Apagar)", min_value=0, step=1, value=0)
    nome = st.text_input("Nome do animal ")
    preco = st.number_input("Peso do animal ", min_value=0.0, format="%.2f")
...
if btn_criar:
    ...
    res = requests.post(f"{'https://projeto-crud-fastapi-1.onrender.com'}/items/", json=payload)
...
if btn_atualizar and id_item > 0:
    payload = {"nome": nome, "preco": preco, "descricao": descricao}
    res = requests.put(f"{'https://projeto-crud-fastapi-1.onrender.com'}/items/{id_item}", json=payload)
    if res.status_code == 200:
...
if btn_deletar and id_item > 0:
    res = requests.delete(f"{'https://projeto-crud-fastapi-1.onrender.com'}/items/{id_item}")
    if res.status_code == 200:
...
try:
    resposta_get = requests.get(f"{'https://projeto-crud-fastapi-1.onrender.com'}/items/")
